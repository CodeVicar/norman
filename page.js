...
const Home = () => {
  ...
  useEffect(() => {
    const enableStream = async () => {
      if (!recordingType || recordedVideoUrl) return;
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        // Ensure the stream is stopped before processing the recorded data
        if (streamRef.current) {
          streamRef.current.getTracks().forEach(track => track.stop());
          streamRef.current = null;
        }
  ...
  const stopRecording = useCallback(() => {
    if (mediaRecorderRef.current && isRecording) {
      ...
      // State updates and stream cleanup will now primarily happen in mediaRecorder.onstop
    }
  }, [isRecording]);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="relative bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className={`w-full bg-black flex items-center justify-center ${
            recordedVideoUrl ? 'h-auto' : 'aspect-video md:h-[600px]'
          }`}>
        ...
        <PlayerModal
          ...
          onClose={() => {
            setIsPlayerModalOpen(false);
            // Optionally reset recording state or prompt for new recording here
            // For now, clicking close on modal just closes modal
          }}
        />
      </div>
    </div>
